      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4">
            {Object.keys(products).length==0?'Sorry currently out of stock..Stay Tuned':''}
...
for (let item of products) {
    if (item.title in Smartphones) {
        if (!Smartphones[item.title].color.includes(item.color) &&item.availableQty>0) {
          Smartphones[item.title].color.push(item.color)
              
        }
         if (!Smartphones[item.title].size.includes(item.size)&&item.availableQty>0) {
          Smartphones[item.title].size.push(item.size)
        }
    }
    else{
      Smartphones[item.title]=JSON.parse(JSON.stringify(item));
        if (item.availableQty>0)
        Smartphones[item.title].color=[item.color];
        Smartphones[item.title].size=[item.size];
        
       }
       
    }
    return {
     props: {products:JSON.parse(JSON.stringify(Smartphones))}, // will be passed to the page component as props
   }
 }
