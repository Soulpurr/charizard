...
function myOrder({ order }) {
  ...
  return (
    <div className="p-3">
      ...
      {order.map((item) => (
        <div
          className="border-2 border-solid w-auto border-gray-400 p-3 mt-4 rounded-xl overflow-auto"
          key={item._id}
        >
          <div className="content ">
            {Object.keys(item.product).map((detail) => (
              <div
                className="top flex flex-row justify-between"
