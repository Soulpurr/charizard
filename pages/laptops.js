...
for (let item of products) {
    if (item.title in laptops) {
        if (!laptops[item.title].color.includes(item.color) &&item.availableQty>0) {
         ...
         if (!laptops[item.title].size.includes(item.size)&&item.availableQty>0) {
    ...
    return {
     props: {products:JSON.parse(JSON.stringify(laptops))}, // will be passed to the page component as props
   }
 }
